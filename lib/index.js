...
module.exports = function bloggifyFlexibleRouter (config, cb) {
    ...
    const setUpErrorPages = errorPages => {
        ...
        if (notFound) {
            ...
            errorPages.notFound = (ctx, data) => ctx.end(`404 – ${data && data.error && data.error.message || "Not Found."}`)
        }
        ...
        if (serverError) {
            ...
            errorPages.serverError = (ctx, data) => ctx.end(`500 – Internal Server Error.`)
        }
        ...
        if (badCsrf) {
            ...
            errorPages.badCsrf = ctx => ctx.end("Your browser did something unexpected. Try refreshing the page. Contact us if the problem persists.")
        }
    ...
    fileTree(routesRoot, (err, paths) => {
        ...
        forEach(uris, (cUri, uri) => {
            const controller = maybeRequire(cUri.controller, true)
